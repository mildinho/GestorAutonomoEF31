...
{
    public class ProdutoSaldoRepository : IProdutoSaldoRepository
    {
        private readonly IConfiguration _conf;
        private readonly GestorAutonomoContext _context;

        public ProdutoSaldoRepository(GestorAutonomoContext context, IConfiguration configuration)
        {
            _context = context;
            _conf = configuration;
        ...
        public async Task DeletarAsync(int Id)
        {
            try
            {
                ProdutoSaldo obj = await SelecionarPorCodigoAsync(Id);
                if (obj != null)
                {
        ...
        public async Task InserirAsync(ProdutoSaldo produtoSaldo)
        {
            produtoSaldo = AjustarCampos(produtoSaldo);
            _context.Add(produtoSaldo);
            await _context.SaveChangesAsync();
        }
        ...
        public async Task<ProdutoSaldo> SelecionarPorCodigoAsync(int? Id)
        {
            return await _context.ProdutoSaldo.FirstOrDefaultAsync(obj => obj.Id == Id);
        }
        ...
        public async Task<List<ProdutoSaldo>> ObterProdutosPorPonto(int Id)
        {
            return await _context.ProdutoSaldo.Where(obj => obj.PontosEstoqueId == Id).ToListAsync();
        }

        public async Task<List<ProdutoSaldo>> ObterPontosPorProduto(int Id)
        {
